...
public:
    ...
    void update(){
        ...
        if(showNDI){
			...
			if (1) {
				...
				if(1){
					int xres = receiver.fbo.getWidth();
					int yres = receiver.fbo.getHeight();
					if (xres != 0 && yres != 0) {
						
						fboReader.readToPixels(receiver.fbo, fboPix);				
						currentMat = ofxCv::toCv(ofImage(fboPix));
    ...
    void findContour(){
        ...
        if(finalImage.bAllocated){
            contourFinder.findContours(foregroundMat, minArea*minArea, maxArea*maxArea, maxBlobNum, bFindHoles, bSimplify);
    ...
    void drawFbo(){
        ...
        if(bDetectBlob){
            ...
            for(int i=0; i<nBlobs; i++){
                ...
                if(minAge <= age){
                    ...
                    // Effects
                    float heatRadius = ofMap(area, 0, 1.0, 10, 50);
        ...
        if(bGlitch){
            ...
            ofClear(0,0,0,0);
            ofSetColor(255);
            ofxCv::drawMat(currentMat, 0,0,w,h);
    ...
    void draw(){
		...
		ofDrawBitmapString(receiver.getInfo(), 0, 25);
